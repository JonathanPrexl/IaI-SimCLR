...
def load_s1_image_BioMasters(path):

    """
    Just valid for this competition
    since the S1 data has four bands here
    """
    ...
    polarizationnames = ["VV_asc","VH_asc","VV_des","VH_des"]
    xarr = xr.DataArray(d, dims=("polarization","lat", "long"),
                           coords={"polarization": polarizationnames})

    return xarr

def load_s2_image_BioMasters(path):

    """
    Just valid for this competition
    since the S2 data has the Cloudmask as and aswell
    """
    ...
    bandnames = ["B02","B03","B04","B05","B06","B07","B08","B08A","B11","B12","CLP"]
    xarr = xr.DataArray(d, dims=("channel","lat", "long"), coords={"channel": bandnames})
